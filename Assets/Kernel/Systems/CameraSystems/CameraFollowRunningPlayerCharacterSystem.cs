...
{
    public class CameraFollowRunningPlayerCharacterSystem : IExecuteSystem
    {
        ...
        public CameraFollowRunningPlayerCharacterSystem(GameContext gameContext, ITime time)
        {
            _time = time;
            _playerCharacters = gameContext.GetGroup(AllOf(ID, PlayerCharacter, Position, Running));
            _followingCameras = gameContext.GetGroup(AllOf(ID, GameMatcher.Camera, FollowingPlayerCharacter, FollowingEntityID, Position, Movable, FollowingOffset, FollowSpeed));
        }
        ...
        public void Execute()
        {
            ...
            foreach (var camera in _followingCameras)
            {
                ...
                var rigidPosition = playerCharacter.position.Value.SetX(0) + camera.followingOffset.Value;
